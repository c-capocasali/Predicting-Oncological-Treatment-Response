...
def run(args: argparse.Namespace) -> list[dict]:
    ...
    # --- NOVO: Valores de frações de treino para análise ---
    # Focado em regimes de poucos rótulos (few-labels): 2% a 30%
    fractions = [0.02, 0.05, 0.10, 0.15, 0.20, 0.30]
    ...
    print(
        f"Protocolo Análise de Split: Frações={
            fractions}, {args.genes} genes, "
        f"K={args.graph_k}, {len(seeds)} seeds"
    )

    for position, seed in enumerate(seeds, start=1):

        # --- NOVO: Loop interno iterando sobre as frações de treino ---
        for fraction in fractions:
            ...
            for model_name in MODEL_ORDER:
                ...
                rows.append(
                    {
                        "seed": seed,
                        "train_fraction": fraction,  # --- NOVO: Registra a fração ---
        ...
        print(f"seed {position:02d}/{len(seeds)
                                     } processada para todas as {len(fractions)} frações.")

    # --- NOVO: Plota o Gráfico de Desempenho vs Fração de Treino ---
    plt.figure(figsize=(10, 6))

    for model_name in MODEL_ORDER:
        model_rows = [row for row in rows if row["model"] == model_name]
        # Agrupa a Acurácia Balanceada (BA) média por fração de treino
        mean_ba = [np.mean([r["balanced_accuracy"]
                           for r in model_rows if r["train_fraction"] == frac]) for frac in fractions]

        plt.plot(fractions, mean_ba, marker='o', label=model_name)

    plt.title(f"Acurácia Balanceada Média vs. Fração de Treinamento\n({
              len(seeds)} seeds, {args.genes} genes, K={args.graph_k})")
    plt.xlabel("Fração de Treinamento (Proporção de Rótulos)")
    plt.ylabel("Acurácia Balanceada Média (BA)")
    plt.xticks(fractions, labels=[f"{f:.0%}" for f in fractions])
    plt.legend()
