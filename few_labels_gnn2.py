...
def run(args: argparse.Namespace) -> list[dict]:
    ...
    # --- NOVO: Valores de K para análise ---
    k_values = [3, 5] + list(range(10, 121, 10))
    ...
    print(
        f"Protocolo Análise de K: Ks={k_values}, "
        f"{args.train_fraction:.0%} rotulados, "
        f"{args.genes} genes, {len(seeds)} seeds"
    )
    ...
    for position, seed in enumerate(seeds, start=1):
        ...
        # --- NOVO: Loop interno para iterar sobre K ---
        for k in k_values:
            # O grafo e as features propagadas são gerados para o K atual
            adjacency = build_patient_graph(features, k)
            appnp_features = propagate_appnp(
                features, adjacency, alpha=args.appnp_alpha, hops=args.appnp_hops
            )
            sgc_features = propagate_sgc(
                features, adjacency, hops=args.sgc_hops
            )
            ...
            models_and_features = {
                ...
                "RBF-SVM": (
                    SVC(
                        C=1.0, gamma=0.001, kernel="rbf", class_weight="balanced",
                        cache_size=2000, random_state=seed,
                    ).fit(features[labelled], labels[labelled]),
                    features,
                ),
                "KNN": (
                    KNeighborsClassifier(n_neighbors=k, metric="cosine").fit(
                        features[labelled], labels[labelled]),
                    features,
                ),
                "Random-Forest": (
                    RandomForestClassifier(
                        class_weight="balanced", random_state=seed).fit(features[labelled], labels[labelled]),
                    features,
                ),
                "MLP": (
            ...
            for model_name in MODEL_ORDER:
                ...
                rows.append(
                    {
                        "seed": seed,
                        "k": k,  # --- NOVO: Registra o K na tabela de resultados ---
        ...
        # Print simplificado para não poluir o terminal com repetições do K
        print(f"seed {position:02d}/{len(seeds)
                                     } processada para todos os {len(k_values)} Ks.")

    # --- NOVO: Plota o Gráfico de Desempenho vs K ---
    plt.figure(figsize=(10, 6))

    for model_name in MODEL_ORDER:
        model_rows = [row for row in rows if row["model"] == model_name]
        # Agrupa a Acurácia Balanceada (BA) média por valor de K
        mean_ba = [np.mean([r["balanced_accuracy"]
                           for r in model_rows if r["k"] == k]) for k in k_values]

        if model_name in ("Linear-sem-grafo", "RBF-SVM", "Random-Forest", "MLP"):
            # Baselines não dependem de K, então geram uma linha reta tracejada constante
            plt.plot(k_values, mean_ba, linestyle='--',
                     label=model_name, alpha=0.7)
        else:
            plt.plot(k_values, mean_ba, marker='o', label=model_name)

    plt.title(f"Acurácia Balanceada Média vs. Tamanho da Vizinhança (K)\n({
              len(seeds)} seeds, {args.genes} genes)")
    plt.xlabel("K do Grafo (Número de Vizinhos)")
    plt.ylabel("Acurácia Balanceada Média (BA)")
    plt.xticks(k_values)
    plt.legend()
    plt.grid(True, alpha=0.4)
    plt.tight_layout()
    plt.savefig("desempenho_vs_k.pdf", format="pdf")
    plt.close()
