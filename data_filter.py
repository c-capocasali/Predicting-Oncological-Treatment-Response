...
def _(mo):
    mo.md(r"""
    ## Pipeline
    - `survival_time`: dias até a morte (`days_to_death`) ou até o último contato
      (`days_to_last_follow_up`) para pacientes censurados.
    - `event`: 1 = óbito confirmado; 0 = censurado (paciente ainda vivo ou perdido
      de seguimento).
    """)
    return
    ...
    def compute_survival_columns(df_clinical: pl.DataFrame) -> pl.DataFrame:
        ...
        return (
            ...
            .select([
                ...
                pl.col("demographic.days_to_death").cast(pl.Float64, strict=False).alias("days_to_death"),
                pl.col("diagnoses.0.days_to_last_follow_up").cast(pl.Float64, strict=False).alias("days_to_last_follow_up")
            ])
            .with_columns([
                # event = 1 apenas quando morreu E temos o tempo da morte
                pl.when(
                    (pl.col("vital_status") == "Dead") & pl.col("days_to_death").is_not_null()
                )
                .then(pl.lit(1))
                .otherwise(pl.lit(0))
                .alias("event"),
...
@app.cell
def _(compute_survival_columns, find_paths, get_rna_exp, pl):
        ...
        for project in project_names:
            project_results = (
                ...
                .agg([
                    ...
                    pl.col("file_name").alias("file_names") # Cria uma lista de arquivos por paciente
                ])
                .collect()
            )
