...
def build_patient_graph(features: np.ndarray, k: int = 3) -> sparse.csr_matrix:
    """KNN expandido, Reranking via RBO, RBF local e normalizacao simetrica."""

    # 1. Busca Expandida: trazemos mais vizinhos para criar um "pool" de candidatos
    pool_size = max(k * 5, 20)
    search = NearestNeighbors(
        n_neighbors=pool_size + 1, metric="cosine", n_jobs=-1
    ).fit(features)
    distances, indices = search.kneighbors(features)

    # Removemos o self-loop inicial
    distances, indices = distances[:, 1:], indices[:, 1:]

    n_nodes = len(features)
    final_indices = np.zeros((n_nodes, k), dtype=int)
    final_distances = np.zeros((n_nodes, k), dtype=np.float32)

    # 2. Reranking da Topologia com o RBO
    for i in range(n_nodes):
        lista_i = indices[i]  # O ranking original do paciente i
        rbo_scores = []

        for j_idx, j in enumerate(lista_i):
            lista_j = indices[j]  # O ranking original do candidato j
            # Calcula a similaridade topológica entre i e j
            score = compute_rbo(lista_i, lista_j, pool_size)
            rbo_scores.append(score)

        # Ordena os candidatos do maior RBO (mais similares) para o menor
        best_candidates = np.argsort(rbo_scores)[::-1][:k]

        # Seleciona os top K candidatos definitivos para formar o grafo
        final_indices[i] = indices[i][best_candidates]
        final_distances[i] = distances[i][best_candidates]

    # 3. Fluxo original reconstruindo o grafo com a nova topologia
    rows = np.repeat(np.arange(n_nodes), k)
    columns = final_indices.reshape(-1)

    local_scale = np.maximum(final_distances[:, -1], 1e-8)
    denominator = local_scale[rows] * local_scale[columns]

    weights = np.exp(
        -np.square(final_distances.reshape(-1)) /
        np.maximum(denominator, 1e-12)
    ).astype(np.float32)

    directed = sparse.csr_matrix(
        (weights, (rows, columns)), shape=(n_nodes, n_nodes)
    )
